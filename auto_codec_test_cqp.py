...
def get_data_from_txt_KSC265(filename, txtfile, outdatafile, anchor='1'):
	...
	oneline = filename + '(anchor)' + ' '*(30-len(filename)+20) + \
	framenum + 10*' ' + bitrate + 10*' ' + str(Y_PSNR) + 10*' ' +\
	str(U_PSNR) + 10*' ' + str(V_PSNR) + 10*' ' + time + 10*' ' +  fps + '\n'
...
def process_encode_decode(rawDemo, srcBinDir, outFileDir, codec='0', scc='0', gprof='0', yuvflag='0', refDemo='0', memcheckflag='0', startIdx=0, saveBin=0):
	...
	for filename in files: #遍历每个码流文件
		...
		if codec == '0' or codec == '1' or codec == '4' or codec == '5':
		...
		for qp in qp_values:
			...
			outrefyuv = 'decoded_' + onlystreamname + '_qp_' + qp + '.yuv'

			# 1.原始可执行文件编码
			if yuvflag != '0':  ##注：输出重建YUV文件(当前没有加入命令行，按需修改)
				# cmd for YL265
				cmd_raw = space.join([rawDemo, width, height, framenum, '0', '2000', qp, filename, outrawstr, '>', outrawtxt, '2>&1'])
			else:
				# cmd for x264 / yl264
				if codec == '0':
					cmd_raw = space.join([rawDemo, '--preset', 'veryfast', '--tune', 'psnr', 
										'--psnr', '-o', outrawstr,  filename, '--input-res', input_res,
										'--fps 30 --keyint 10000 --qp', qp, '--threads 1  --bframes 3', '>', outrawtxt, '2>&1']) 								  
				# cmd for x265
				elif codec == '1':
					 cmd_raw = space.join([rawDemo, '--input', filename, '--preset', 'veryfast', '-t', 'zerolatency', 
					 		'--psnr', '-o', outrawstr, '--input-res', input_res, '--qp', qp, '-f ', framenum,
					 		'--fps 30 --keyint 10000 --aq-mode 0 --no-psy-rd -b 0', '>', outrawtxt, '2>&1']) #--no-wpp -F 1
#					 cmd_raw = space.join([rawDemo, '--input', filename, '--preset', 'veryfast', '-t', 'zerolatency', 
#					 		'--psnr', '-o', outrawstr, '--input-res', input_res, '--qp', qp, '-f', framenum,
#					 		'--fps 30 --keyint 0 --aq-mode 0 --no-psy-rd -b 0', '>', outrawtxt, '2>&1']) #--no-wpp -F 1
				...
				# cmd for KSC265
					...
					cmd_raw = space.join([rawDemo, '-tune default -preset veryfast -latency zerolatency', '-i', filename, ' -fr 30 -iper 10000',  '-frms', framenum, #-iper 300 -imin 300
							 '-b', outrawstr, '-wdt', width, '-hgt', height, '-rc 0  -fpp 0 -bframes 0 -fixqp 1 -threads 1 -wpp 0', '-qp', qp, '>', outrawtxt, '2>&1'])	 # -sao 0 -df 0	-threads 1 -wpp  -ref0 1 -ref 1	
#					cmd_raw = space.join([rawDemo, '-tune default -preset veryfast -latency zerolatency', '-i', filename, ' -fr 30 -iper 1',  '-frms', framenum, #-iper 300 -imin 300
#							 '-b', outrawstr, '-wdt', width, '-hgt', height, '-rc 0  -fpp 0 -bframes 0 -fixqp 1 -threads 1 -wpp 0', '-qp', qp, '>', outrawtxt, '2>&1'])	 # -sao 0 -df 0	-threads 1 -wpp  -ref0 1 -ref 1	
				# cmd for YL265
				elif codec == '5':
					cmd_raw = space.join([rawDemo, width, height, framenum, scc, '2000', qp, filename, outrawstr, '>', outrawtxt, '2>&1']) # 0: 主流 1: 辅流  framenum
			print(cmd_raw)
			...
			if refDemo != '0':
				if yuvflag != '0':
					...
					cmd_ref = space.join([refDemo, '--input', filename, '--preset medium', '-t psnr', 
									...
									'--fps 30 --no-wpp -F 1 -I 0 -b 0 -f 20 --qp', qp, '>', outreftxt, '2>&1'])
				print(cmd_ref)
			...
			if codec == '0':
				get_data_from_txt_x264(stream_name_res+'_qp'+qp, outrawtxt, outtotal, 1)
				if(refDemo != '0'):
					get_data_from_txt_x264(stream_name_res, outreftxt, outtotal, 0)
			elif codec == '1':
				get_data_from_txt_x265(stream_name_res+'_qp'+qp, outrawtxt, outtotal, 1)
				#if(refDemo != '0'):
				#	get_data_from_txt_x265(stream_name_res+'_qp'+qp, outreftxt, outtotal, 0)
			elif codec == '2':
				get_data_from_txt_uavs3e(stream_name_res+'_qp'+qp, outrawtxt, outtotal, 1)
				if(refDemo != '0'):
					get_data_from_txt_uavs3e(stream_name_res, outreftxt, outtotal, 0)					              
			elif codec == '3':
				get_data_from_txt_vvenc(stream_name_res+'_qp'+qp, outrawtxt, outtotal, 1)
				if(refDemo != '0'):
					get_data_from_txt_vvenc(stream_name_res, outreftxt, outtotal, 0)					              
			elif codec == '4':
				get_data_from_txt_KSC265(stream_name_res+'_qp'+qp, outrawtxt, outtotal, 1)
			elif codec == '5':
				get_data_from_txt_yl265(stream_name_res+'_qp'+qp, outrawtxt, outtotal, 1)
