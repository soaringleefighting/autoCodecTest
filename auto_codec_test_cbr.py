...
def get_data_from_txt_x265(filename, txtfile, outdatafile, anchor='1'):
	...
	if(anchor==1):
		oneline = filename + '(anchor)' + ' '*(30-len(filename)+10) + \
				framenum + 10*' ' + bitrate + 10*' ' + PSNR + 10*' '+ time + '\n'
	else:
...
def get_data_from_txt(filename, txtfile, outdatafile, anchor='1'):
	...
	Data = {}  #dictory
	for i in range(len(lines)):
		if lines[i].find('PSNR Y') != -1:
                    PSNR=lines[i].split(':')[1]
                if lines[i].find('bitrate')!= -1:
                    bitrate = lines[i].split(':')[1]
                if lines[i].find('Encoded frame count')!= -1:
                    framenum = lines[i].split('=')[1]
...
def collect_data_to_excel(excelname, inputfile, anchor='1'):
	...
	for i in range(len(lines)):
		if lines[i].find('anchor') != -1 or lines[i].find('ref') != -1:
			...
			#print filename
			data[filename] = [filename, splitValue[1], splitValue[2], splitValue[3], splitValue[4]]
	...
	if count==0:  ##第一次打开文件时才写入
		title=['video sequence', 'total frames', 'bitrate(kbps)', 'PSNR(dB)', 'time(s)']
...
def process_encode_decode(rawDemo, srcBinDir, outFileDir, gprof='0', yuvflag='0', refDemo='0', memcheckflag='0'):
	...
	for filename in files: #遍历每个码流文件
		...
		totaltitle = 'filename' + ' '*(42 - len('#filename') + 10) + 'total_frames'+ 10*' ' + 'bitrate'  + 10*' ' + 'PSNR' + 10*' ' + 'time(s)'
