...
##脚本用法： python auto_codec_coherence.py rawDemo srcStreamDir outFileDir [gprofflag outyuvflag refDemo]
...
Anchor_memchecklog = '_pyAnchor_memcheck' #对待验证可执行文件进行内存检在查后汇总文件
...
#编码或解码处理			
def process_encode_decode(rawDemo, srcBinDir, outFileDir, gprof='0', yuvflag='0', refDemo='0'):
...
if __name__ == '__main__':
    ...
    ret = process_encode_decode(rawDemo, srcStreamDir, outFileDir, gprof, yuvflag, refDemo)
    if (ret!=0):
	exit()
