...
def get_data_from_txt_x265(filename, txtfile, outdatafile, anchor='1'):
	...
	for i in range(len(lines)):
            ...
            if lines[i].find('frame P:') != -1:
                Mean_PSNR = lines[i].strip('\n').split('PSNR Mean:')[-1]
                Y_PSNR_P    = Mean_PSNR.split(' ')[1].split('Y:')[1]
                U_PSNR_P    = Mean_PSNR.split(' ')[2].split('U:')[1]
                V_PSNR_P    = Mean_PSNR.split(' ')[3].split('V:')[1]
                Y_PSNR = (float(Y_PSNR_I) + float(Y_PSNR_P))/2
                U_PSNR = (float(U_PSNR_I) + float(U_PSNR_P))/2
                V_PSNR = (float(V_PSNR_I) + float(V_PSNR_P))/2 
		...
		for br in bitrate:
                          ...
                          if yuvflag != '0':
                              ...
                              cmd_raw = space.join([rawDemo, '-i', filename, '--speed_level 3 --fps_num 30 --fps_den 1', 
                                           ...
                                           '--frm_threads 1 --wpp_threads 1 --rc_type 2 -p 100 -g 0 -v 2 --rc_bitrate', br, '>', outrawtxt, '2>&1'])
                              # cmd for libaom
		          ...
		          ## 将性能数据结果输出到格式化文本中 outrawtxt--->outtotal
                          get_data_from_txt_uave3e(onlystreamname+'_br'+br, outrawtxt, outtotal, 1)
                          if(refDemo != '0'):
